...
See `\Drupal\helfi_api_base\Vault\EnsureApiAccountsSubscriber` event subscriber.
...
The [Key auth](https://www.drupal.org/project/key_auth) module is supported. Add `api_key` to your `accounts`:
...
We hook into `helfi_api_base.post_deploy` event ([src/Vault/EnsureApiAccountsSubscriber.php](/src/Vault/EnsureApiAccountsSubscriber.php)), triggered by `drush helfi:post-deploy` command executed as a part of deployment tasks: [https://github.com/City-of-Helsinki/drupal-helfi-platform/blob/main/docker/openshift/hooks/deploy/20-deploy.sh](https://github.com/City-of-Helsinki/drupal-helfi-platform/blob/main/docker/openshift/hooks/deploy/20-deploy.sh)
